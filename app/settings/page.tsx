  ...
  return (
    <div className="flex flex-col h-screen">
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-8 max-w-2xl">
        ...
        {/* Scenario */}
        <section>
          <h2 className="text-sm font-semibold text-gray-900 mb-3">Scenario</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {SCENARIO_OPTIONS.map((opt) => (
              ...
              >
                <p className={`text-sm font-medium ${config.scenario === opt.id ? "text-blue-700" : "text-gray-900"}`}>
                  {opt.label}
                </p>
        ...
        {/* Time Mode */}
        <section>
          <h2 className="text-sm font-semibold text-gray-900 mb-3">Time Mode</h2>
          <div className="flex gap-3">
            ...
            >
              <p className={`text-sm font-medium ${config.timeMode === "live" ? "text-green-700" : "text-gray-900"}`}>Live</p>
            ...
            >
              <p className={`text-sm font-medium ${config.timeMode === "paused" ? "text-amber-700" : "text-gray-900"}`}>Paused</p>
        ...
        {/* Snapshot Interval */}
        <section>
          <h2 className="text-sm font-semibold text-gray-900 mb-3">Snapshot Interval</h2>
          <select
            value={config.snapshotIntervalMs}
            onChange={(e) => updateConfig({ snapshotIntervalMs: Number(e.target.value) })}
            className="text-sm border border-gray-200 rounded-lg px-3 py-2 text-gray-700 bg-white w-full max-w-xs"
          >
            {INTERVAL_OPTIONS.map((opt) => (
              <option key={opt.value} value={opt.value}>{opt.label}</option>
            ))}
          </select>
          <p className="text-xs text-gray-400 mt-2">How often the live state rotates between snapshots</p>
        </section>
        ...
        <section>
          ...
          <div className="space-y-3">
            <label className="flex items-center justify-between bg-white border border-gray-200 rounded-lg p-4 cursor-pointer">
              <div>
                <p className="text-sm font-medium text-gray-900">Demo Mutation</p>
                <p className="text-xs text-gray-500">Allow in-memory intervention application</p>
              </div>
              <input
                type="checkbox"
            ...
            <label className="flex items-center justify-between bg-white border border-gray-200 rounded-lg p-4 cursor-pointer">
              <div>
                <p className="text-sm font-medium text-gray-900">Runtime Validation</p>
                <p className="text-xs text-gray-500">Validate each snapshot against schema on load</p>
              </div>
              <input
                type="checkbox"

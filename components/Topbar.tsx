  ...
  return (
    <header className="h-14 bg-white border-b border-gray-200 flex items-center justify-between px-3 sm:px-6 shrink-0">
      <div className="flex items-center gap-2 sm:gap-3 min-w-0">
        ...
        <button onClick={toggle} className="lg:hidden p-1.5 -ml-1 rounded-md text-gray-500 hover:bg-gray-100">
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        ...
        <span className="hidden sm:inline text-xs font-mono text-gray-600">{formattedTime}</span>
        {session && (
          <div className="flex items-center gap-2 sm:gap-3 border-l border-gray-200 pl-2 sm:pl-4">
            <span className="hidden sm:inline text-xs text-gray-500 capitalize">{session.username}</span>
            <button
              onClick={logout}
              className="flex items-center gap-1.5 text-xs px-2 sm:px-2.5 py-1 rounded-md border border-gray-200 text-gray-500 hover:text-red-600 hover:border-red-200 hover:bg-red-50 transition-colors"
            >

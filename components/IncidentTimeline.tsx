  ...
  if (!events || events.length === 0) {
    return (
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-5">
        <p className="text-sm text-gray-400 dark:text-gray-500">No events to display</p>
      </div>
    );
  }
          ...
          {events.map((event) => {
            ...
            return (
              <div key={event.event_id} className="relative pl-8">
                <div
                  className={`absolute left-1.5 top-1.5 w-3 h-3 rounded-full ${dotColor} ring-2 ring-white dark:ring-gray-900`}
                />
                <button
                  onClick={() =>
                ...
                {isExpanded && event.payload && (
                  <div className="mt-2 bg-gray-50 dark:bg-gray-800 rounded-md p-3 text-xs font-mono text-gray-600 dark:text-gray-400 overflow-x-auto">

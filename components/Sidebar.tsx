...
import { isAuthorizedForPath, getDefaultRoute } from "@/lib/auth/types";
...
const NAV_SECTIONS: NavSection[] = [
  ...
  },
  {
    ...
    items: [
      { href: "/analytics", label: "Traffic Analytics", icon: "chart" },
    ],
  },
];
...
export default function Sidebar() {
  ...
  return (
    <aside className="w-56 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 flex flex-col min-h-screen shrink-0">
      <div className="px-5 py-5 border-b border-gray-200 dark:border-gray-700">
        <Link href={session ? getDefaultRoute(session.role) : "/login"} className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-blue-600 flex items-center justify-center">
            <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
            </svg>
          </div>
          <span className="font-semibold text-gray-900 dark:text-gray-100 text-sm">SchoolZone</span>
        </Link>
      </div>
      ...
      <div className="px-3 py-3 border-t border-gray-200 dark:border-gray-700 space-y-2">
          ...
          <div className="px-3 py-2 space-y-2">
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center">
                <span className="text-[10px] font-bold text-gray-600 dark:text-gray-300 uppercase">{session.username[0]}</span>
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium text-gray-900 dark:text-gray-100 truncate">{session.username}</p>
            ...
            <button
              onClick={logout}
              className="w-full flex items-center gap-2 px-2 py-1.5 text-xs text-gray-500 dark:text-gray-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-950 rounded-md transition-colors"
            >
        ...
        <p className="px-3 text-[10px] text-gray-400 dark:text-gray-500">Digital Twin v2.0</p>
      </div>
    </aside>
  );
}

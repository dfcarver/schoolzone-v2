...
The platform supports two data modes, switchable via environment variable:

- **Live mode** — telemetry is generated in real time by the AWS data pipeline (`infra/`), stored in DynamoDB, and served to the app via a Lambda Function URL. Set `NEXT_PUBLIC_AWS_SNAPSHOT_API_URL` to enable.
- **Demo mode** — all telemetry, incidents, and snapshot data are served from static mock JSON files (`/public/mock/`). The demo is deterministic and reproducible, requiring no live sensor infrastructure. This is the default when the env var is not set.

`loadLiveState()` in `lib/data.ts` checks for the env var and falls back to mock data automatically if the AWS API is unavailable.
...
| Hosting | Vercel | — |
| Infrastructure-as-Code | AWS CDK v2 (TypeScript) | 2.1108.0 |
| Data Pipeline | AWS (Kinesis, Firehose, S3, DynamoDB, Lambda, EventBridge) | — |
...
│  /public/mock/*.json  │  OpenAI · Google Maps · Supabase     │
│  (Demo / fallback)    ├──────────────────────────────────────┤
│                       │        AWS Data Pipeline             │
│                       │  EventBridge → Generator Lambda      │
│                       │       → Kinesis Data Stream          │
...
## 8. AWS Data Pipeline (`infra/`)

The `infra/` directory contains a CDK v2 TypeScript project that provisions a real-time traffic data generation and storage pipeline on AWS. It is optional — the app falls back to static mock data when not configured.
...
EventBridge Scheduler (every 60s)
    │
    ▼
Generator Lambda              — Gaussian traffic model for all 11 zones
    │
    ▼
Kinesis Data Stream
    ├── Kinesis Firehose ──► S3 Data Lake    (cold path, Athena-queryable)
    └── Processor Lambda ──► DynamoDB        (hot path, latest state per zone)
                                 │
                            Snapshot API Lambda
                                 │
                            Lambda Function URL (HTTPS, CORS-enabled)
                                 │
                         Next.js app (via NEXT_PUBLIC_AWS_SNAPSHOT_API_URL)
```
...
|----------|------|---------|
| EventBridge Rule | `schoolzone-generator-schedule` | Triggers generator every 60 seconds |
| Lambda | `schoolzone-generator` | Computes zone telemetry, publishes to Kinesis |
| Kinesis Data Stream | `schoolzone-traffic` | Real-time telemetry transport (1 shard, 24h retention) |
| Kinesis Firehose | `schoolzone-to-datalake` | Buffers stream → S3 (60s/5MB, GZIP, date-partitioned) |
| S3 Bucket | `schoolzone-datalake-<account>-<region>` | Data lake (IA at 30d, Glacier at 90d) |
| DynamoDB Table | `schoolzone-zone-state` | Latest zone telemetry per city/zone; 10-min TTL |
| Lambda | `schoolzone-processor` | Kinesis → DynamoDB writer |
| Lambda | `schoolzone-snapshot-api` | Serves LiveState JSON to the Next.js app |
| Lambda Function URL | — | HTTPS endpoint for the snapshot API |

### Generator Traffic Model

The Generator Lambda runs the same probabilistic model as the front-end congestion engine:

- **Vehicle count** — Gaussian peaks at dismissal time (school-type dependent) and morning drop-off (7:45 AM), scaled by enrollment × 65% pick-up rate
- **Dismissal times** — Elementary 3:00 PM, Middle 3:15 PM, High 3:30 PM
- **Speed** — 25 mph free-flow, reduced by congestion (up to −15 mph) and weather
- **Pedestrian count** — Gaussian peak at dismissal, scaled by enrollment × 35%
- **Risk score** — Weighted: congestion (50%) + speed factor (30%) + pedestrian density (20%)
- **Weather** — Controlled via `WEATHER_CONDITION` env var on the Generator Lambda (`clear`, `rain`, `fog`)

### DynamoDB Schema

| Attribute | Type | Description |
|-----------|------|-------------|
| `city_id` | String (PK) | `springfield_il`, `khalifa_city_auh`, `mbz_city_auh` |
| `zone_id` | String (SK) | e.g. `zone-001`, `khalifa-002` |
| `zone_name` | String | Human-readable name |
| `timestamp` | String | ISO timestamp of the reading |
| `risk_score` | Number | 0–1 |
| `vehicle_count` | Number | Vehicles observed |
| `pedestrian_count` | Number | Pedestrians observed |
| `speed_avg_mph` | Number | Average speed |
| `congestion_index` | Number | 0–1 |
| `active_cameras` / `total_cameras` | Number | Camera counts |
| `weather` | String | Active weather condition |
| `ttl` | Number | Unix epoch; DynamoDB auto-deletes after 10 minutes |

### Snapshot API

- **Endpoint:** Lambda Function URL (HTTPS, no auth, CORS open)
- **Method:** `GET`
- **Query params:** `?city=<cityId>` and `?weather=<condition>`
- **Response:** `LiveState` JSON — identical shape to the static mock files
- **Fallback:** If DynamoDB has no records (e.g. before the first generator run), the Lambda computes the response directly from the traffic model

### S3 Data Lake Layout

```
s3://schoolzone-datalake-<account>-<region>/
└── raw/
    └── year=2026/month=03/day=01/hour=14/
        └── schoolzone-to-datalake-1-2026-03-01-14-00-00-<uuid>.gz
```

Records are newline-delimited JSON (NDJSON), queryable via AWS Glue + Athena.

### App Integration

`lib/data.ts` — `loadLiveState()` checks `NEXT_PUBLIC_AWS_SNAPSHOT_API_URL`:

1. If set → `GET <url>?city=<cityId>&weather=<condition>`
2. On failure → falls back to static mock JSON (silent, logs a warning)
3. If not set → static mock JSON directly

No other changes to the application are required.
...
cdk deploy      # ~2 minutes
```

`cdk deploy` outputs the `SnapshotApiUrl` to copy into `.env.local`.
...
│           ├── risk.json
│           └── integration.json
├── infra/                              # AWS CDK data pipeline
│   ├── bin/app.ts                      # CDK app entry point
│   ├── lib/schoolzone-stack.ts         # Stack: Kinesis, S3, DynamoDB, Lambdas
│   ├── lambdas/
│   │   ├── generator/
│   │   │   ├── index.ts               # Lambda handler — publishes to Kinesis
│   │   │   ├── model.ts               # Gaussian traffic model + forecast
│   │   │   └── zones.ts               # Zone definitions (all 3 cities, 11 zones)
│   │   ├── processor/
│   │   │   └── index.ts               # Kinesis → DynamoDB writer
│   │   └── snapshot-api/
│   │       └── index.ts               # DynamoDB → LiveState JSON API
│   ├── package.json
│   ├── tsconfig.json
│   ├── cdk.json
...
| **Timestream** | Amazon Timestream for LiveAnalytics requires special account enablement for new AWS accounts. DynamoDB is used instead as the hot store. Timestream can be re-introduced once enabled. |
